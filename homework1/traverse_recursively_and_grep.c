/*3. Аналог grep. Написать программу, которая на вход получает подстроку и путь,и рекурсивно обходя 
все файлы под заданным путем, печатает те строки и номера строк, которые содержат этот шаблон.*/
